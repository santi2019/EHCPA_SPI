...
## Funcion automatic_s3_uploader: Sirve para la generar el backup de los archivos de precipitacion mensual acumulada del directorio
#  "IMERG_late_month".
...
#
#  2. Listamos la canidad de archivos que hay actualmente en el directorio "IMERG_late_month". Filtramos la lista de archivos para 
#     identificar solo con aquellos que contienen la palabra "IMERG" en su nombre para asegurar de que solo se suban los archivos 
#     de precipitacion mensual acumulada.
#  3. Iteramos sobre cada uno de los archivos para subirlos al servicio S3 de AWS, y para esto utilizamos la funcion "upload_file()" 
#     en la cual establecemos la ruta local completa del archivo a subir, luego definimos el bucket donde se cargaran los archivos, 
#     y este es el que extraemos de las variables de entorno, y construimos la key o ruta de S3 para los archivos, que consiste en 
#     el prefijo (prefix), es decir, el nombre de la carpeta dentro del bucket, sumado el nombre del archivo. Después de cargar cada 
