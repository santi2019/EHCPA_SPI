...
def download_subset(begTime, endTime, reset_ARG_late):
    ...
    #               caso son las imagenes IMERG de tipo Late Day version 7, luego definimos el corte espacial, es decir,
    #               las coordenadas para abarcar todo el territorio Argentino, y por ultimo especificamos la variable de
    #               interes, que es la precipitacion. Para el caso de las fechas de descarga, estas son recibidas por 
    ...
    #          los datos resultantes del paso anterior.
    #
    ...
    #            de ser descargadas.
    #          
    #          1. Formateo de carpeta ARG_late: en caso de que la variable "reset_ARG_late", pasada por parametro a la 
    #             funcion, sea "True", se verifica:
    #             - Si la fecha actual es igual al tercer dia del proximo mes, tomando como base de comparacion, el mes 
    #               siguiente al del mes actual, guardandolo en una variable para realizar la comparacion ya que sino 
    #               siempre estariamos un mes adelantado. Cuando se cumple esta condicion, se verifica:
    #               - Si la carpeta "ARG_late" existe y si es un directorio, y en caso de que sea asi, dicha carpeta se 
    ...
    #             Si la variable es "False" no se realizan las verificaciones anteriores y simplemente se procede a realizar 
    #             la descarga.  
    #          2. Descarga de archivos: Como primera medida se imprime un mensaje indicando que se va a mostrar la salida 
    #             de los servicios HTTP, luego iteramos sobre cada URL de la lista de URLs obtenidas del subset, extrayendo 
    #             de cada una el enlace o link y guardandolo en una variable URL. Con "requests.get(URL)", realizamos una 
    #             solicitud HTTP GET a la URL tomada en la iteracion para descargar el contenido del mismo. Se verifica si 
    #             la solicitud fue exitosa, y si el código de estado HTTP indica un error (cualquier valor distinto de 200), 
    #             se lanza una excepcion. Todos los archivos se van a guardar en el directorio "ARG_late", por lo tanto se 
    #             crea un archivo con dicha ruta, y se lo abre en modo escritura binaria con "wb", para escribir sobre el el 
    #             contenido del archivo descargado mediante la operacion la operacion "write", y luego se imprime el nombre 
    #             del archivo creado.
    ...
    ## Manejo de errores: En caso de que ocurra algunos de los errores capturados, se setea la bandera "error_found" 
    #                     como "True", se setea el mensaje de dicho error en "error_message" y se lo imprime.
    #                     Los errores conocidos son:
    #                     - En caso de que el valor de "downloaded_files" sea 0, se indica que no se descargaron datos,
    #                       ya sea por ejemplo, porque no estan disponibles en el sitio GES DISC. 
    #                     - HTTPError: Error en la descarga de archivos IMERG por error de autenticacion.
    #                     - KeyError: Error en la solicitud a la API, por mal seteo de parametros en el json de solicitud
    #                       o el sitio GES DISC esta roto o en mantenimiento.
    #                     - JSONDecodeError: Error en la solicitud a la API, por problema de conectividad local o API 
    #                       fuera de servicio, recibiendo un json con un formato erroneo.
    #                     - NameResolutionError, MaxRetryError: Errores de conectividad, cuando no hay internet.
    #                     - UrllibTimeoutError, RequestsTimeoutError: Errores de tiempo de espera de respuesta excedido 
    #                       a la hora de consultar o enviar el json de solicitud del subset a la API de la NASA. 
