...
## PASO 1: Proceso de verificacion de existencia de archivos de precipitacion mensual acumulada en carpeta local
#          IMERG_late_month, para efectuar o no la descarga del backup del servicio S3 de AWS.  
...
#         - IMERG_late_month_dir: Carpeta donde se guardan los archivos de precipitacion mensual acumulada. En caso
#           de que dicha carpeta no exista, se la crea y se llama a la funcion "download_files()" para efectura la
#           descarga de los archivos en el backup. Caso contrario, listamos la cantidad de archivos que hay en el
#           mismo, luego listamos la cantidad de archivos almacenados en el bucket del servicio S3, y verificamos 
#           que:
#           - Si el número de archivos locales es menor que el número de archivos en S3, se avisa mediante un 
#             mensaje, y se vuelve a llamar a la función "download_files()" para descargar el backup.
...
## PASO 2: Proceso de descarga del backup.
#         - La funcion recibe como parametros la conexion con el cliente S3, el nombre del bucket y el prefix del 
#           backup, y la ruta al directorio "IMERG_late_month".
#         - Se utiliza el cliente de S3 para listar los archivos en el bucket, con "list_objects_v2" nuevamente, en 
#           donde dentro de "response", la clave "Contents" contiene la información sobre cada archivo, y verificamos
#           que:
#           - Si la respuesta contiene informacion de cada archivo, iteramos sobre todos los archivos listados, para
#             extraer el "Key" que es una cadena de texto que representa la ruta completa del archivo dentro del 
#             bucket, y luego se obtiene solo el nombre del archivo, separándolo por / y seleccionando el último 
#             elemento, y se lo almacena. 
#             - Si "file_name", es decir, el nombre del archivo, no está vacío, creamos la ruta de descarga "download_path"
#               y mediante "download_path()" descargamos el archivo de S3 a la ruta especificada. Cuando termina la 
#               descarga de todos los archivos se avisa por mensaje.
